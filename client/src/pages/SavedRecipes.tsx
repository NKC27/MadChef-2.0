...
import type { CurrentUser, SavedRecipe } from '../types';
...
const SavedRecipes = () => {
  ...
  const {
    ...
    error: getMeError,
    refetch,
  } = useQuery<{ me: CurrentUser }>(GET_ME, {
  ...
  const [pendingDelete, setPendingDelete] = useState<SavedRecipe | null>(
    null,
  );

  const userData = data?.me || { savedRecipes: [] as SavedRecipe[] };

  const confirmDelete = (recipe: SavedRecipe) => setPendingDelete(recipe);
